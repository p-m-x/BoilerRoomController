...
String Portal::indexTemplateProcessor(String arg)
{
  if (arg == "RANDOM-HASH")
    return _randomHash;
  if (arg == "NEED-RESTART")
    return String(_needRestart);
  if (arg == "WIFI-SSID")
    return _config.wifiSsid;
  if (arg == "WIFI-PASSWORD")
    return _config.wifiPassword;
  if (arg == "MQTT-HOST")
    return _config.mqttHost;
  if (arg == "MQTT-USERNAME")
    return _config.mqttUsername;
  if (arg == "MQTT-PASSWORD")
    return _config.mqttPassword;
  if (arg == "CONFIG-HA-MQTT-DISCO-TOPIC")
    return _config.configHaMqttDiscoveryTopicPrefix;
  if (arg == "DEFAULT-INPUT-FIELD-LENGTH")
    return String(DEFAULT_INPUT_FIELD_LENGTH);
  if (arg == "HA-MQTT-DISCO-TOPIC-LENGTH")
    return String(HA_MQTT_DISCO_TOPIC_PREFIX_LENGTH);
  return "";
}
